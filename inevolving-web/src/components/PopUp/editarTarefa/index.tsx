...
export default function EditarTarefa( { tarefa, voltar }: { tarefa: Tarefa_Modulo_Tarefas; voltar:() => void } ) {
    ...
    const deletarTarefaRecorrente = async () => {
        ...
        const response = await fetch(
            linkApi+'/auth/api/tasks/repeat/'+tarefa?.id+"/"+tarefa.dateTask, 
            {
            method: 'DELETE',
            headers: {
                'Content-Type': 'application/json',
    ...
    const pegarObjetivo = useCallback( async () => {
            ...
            const response = await fetch(
                    linkApi+'/auth/api/objectives/'+tarefa.idObjective, 
                {
                        method: 'GET',
                        headers: {
                            'Content-Type': 'application/json',
    ...
    const atualizarTarefa = async () => {
            ...
            if (opcaoAtualizar === 1) {
                ...
                const response = await fetch(
                        linkApi+'/auth/api/tasks/repeat/'+tarefa.id+"/"+dataFinal?.toISOString().split('T')[0], 
                    {
                            method: 'PUT',
                            headers: {
                                'Content-Type': 'application/json',
            ...
            if (!(statusDaTarefa === tarefa.status)) {
                if (statusDaTarefa === "TODO") {
                    ...
                    const response = await fetch(
                        linkApi+'/auth/api/tasks/status/canceled', 
                    {
                            method: 'PATCH',
                            headers: {
                                'Content-Type': 'application/json',
            ...
            if (tarefa.dateTask !== dataTarefa?.toISOString()) {
                const response = await fetch(
                    linkApi+'/auth/api/tasks/date/'+tarefa.id+'/'+dataTarefa?.toISOString().split('T')[0], 
                    {
                        method: 'PUT',
                        headers: {
                            'Content-Type': 'application/json',
